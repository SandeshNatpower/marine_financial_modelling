...
def register_callbacks(app):
    ...
    ###########################################################################
    # NEW: PDF GENERATION & EMAIL CALLBACKS
    ###########################################################################
    # Note: We now expect a hidden dcc.Store (id="detailed-report-store")
    # that holds the full HTML of the detailed report (populated elsewhere in your app).
    @app.callback(
        Output("download-report", "data"),
        Output("generated-report-content", "children"),
        Input("generate-report-button", "n_clicks"),
        State("report-type", "value"),
        State("report-dates", "start_date"),
        State("report-dates", "end_date"),
        State("report-filters", "value"),
        State("report-currency", "value"),
        State("detailed-report-store", "data"),  # New: detailed report HTML content
        prevent_initial_call=True
    )
    def generate_pdf_report(n_clicks, report_type, start_date, end_date, filters, currency, detailed_report_html):
        """
        1) Use the detailed report HTML from the UI if available.
        2) If not, fallback to a basic template.
        3) Convert that HTML to PDF bytes with pdfkit.
        4) Return the PDF as a downloadable file via dcc.Download and update the UI preview.
        """
        if not n_clicks:
            return no_update, no_update

        # Use the detailed report HTML if available; otherwise, build a default
        if detailed_report_html:
            html_content = detailed_report_html
        else:
            html_content = f"""
            <html>
            <head>
            ...
            <body>
                ...
                <hr>
                <p>This is a basic PDF generation example from the Decarbonizer tool.</p>
            </body>
            </html>
            """

        # Convert HTML -> PDF (in memory) using the configuration
        try:
            pdf_bytes = pdfkit.from_string(html_content, False, configuration=PDFKIT_CONFIG)
        except Exception as e:
            return no_update, html.P(f"Error generating PDF: {e}", style={"color": "red"})

        # Return the PDF for immediate download
        download_data = dcc.send_bytes(pdf_bytes, "report.pdf")

        # Update preview message
        preview_div = [
            html.P("Detailed report generated successfully. Use the download prompt below:"),
            html.Pre(html_content)
        ]
        
        return download_data, preview_div
    ...
    def send_report_via_email(n_clicks, email_address, report_type, start_date, end_date, filters, currency, detailed_report_html):
        """
        Generate the PDF using the detailed report HTML (or a default) and send it via email.
        """
        if not n_clicks:
            return no_update
        ...
        # Use the detailed report HTML if available; otherwise, build a default template
        if detailed_report_html:
            html_content = detailed_report_html
        else:
            html_content = f"""
            ...
            <body>
                ...
                <p><strong>Currency:</strong> {currency}</p>
                <hr>
                <p>This is a basic PDF generation example sent via email from the Decarbonizer tool.</p>
            </body>
            </html>
            """
        ...
        msg = EmailMessage()
        msg["Subject"] = "Your Detailed Compliance Report"
        msg["From"] = "YOUR_EMAIL_ADDRESS"       # Replace with your sender address
        msg["To"] = email_address
        msg.set_content("Please find attached your detailed compliance report.")
        
        # Add the PDF as an attachment
        msg.add_attachment(pdf_bytes, maintype="application", subtype="pdf", filename="report.pdf")

        # Send the email using proper SMTP settings
        try:
            smtp_server = "smtp.gmail.com"       # Replace with your SMTP server (e.g., smtp.gmail.com)
            smtp_port = 587                        # Use 465 for SSL if needed
            smtp_user = "kirankumarashokpatil@gmail.com"       # Replace with your email address
            smtp_password = "Kirankumar1@"  # Replace with your email password
